...
A local MCP server that watches a log directory and lets Claude read application
logs in real time. Instead of asking the user to copy-paste log output, Claude
queries the logs directly. This is a reusable pattern — not project-specific — and
the server itself is ~200 lines.

## Why

Some runtimes don't put logs where Claude can see them:

- **Tauri / desktop apps** — logs go to a file, not the terminal
- **Background services** — stdout isn't visible to the session
- **Debugging sessions** — Claude needs to see what actually happened at runtime,
...
Claude: [list_logs]  → finds longhand-20260320.log
Claude: [get_errors] → finds "AllSource not initialized"
Claude: "Startup race condition — AllSource hasn't finished initializing when
