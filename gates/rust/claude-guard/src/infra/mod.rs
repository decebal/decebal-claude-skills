//! PreToolUse — deny or prompt by BLAST RADIUS, not by how simple the command
//! looks.
//!
...
//! or asks; a chain that touches an infra tool or prod token in a mutating form
//! with no clean hit asks; everything else is silently allowed. Benign chains
//! never prompt.
...
fn load_prod_tokens(cwd: &str) -> ProdTokens {
    ...
    for base in bases {
        let file = base.join(".claude/prod-guard-tokens.txt");
...
pub fn run(payload: &Value) -> ! {
    match decide(payload) {
        ...
        Decision::Ask(r) => hook::ask(&r),
        // `INFRA_GUARD_OFF=1` downgrades a would-be deny to a prompt — it lets
        // the user approve a normally-blocked command at the prompt. It never
        // silently allows, and a real disable means removing the hook from
        // settings.json.
        Decision::Deny(r) => {
            if std::env::var("INFRA_GUARD_OFF").as_deref() == Ok("1") {
                hook::ask(&format!(
                    "NORMALLY BLOCKED — {r}  (INFRA_GUARD_OFF is set: approve only if you intend this)"
                ));
            }
            hook::deny(&r)
        }
...
fn deep_scan(cmd: &str, cwd: &str, prod_re: Option<&str>) -> Decision {
    ...
    match scan.verdict {
        ...
        None if scan.risk => Decision::Ask(format!(
            "Wrapper chain references an infra tool / prod token with no clean match (dynamic command?). Review it; trust with `claude-guard trust '{cmd}'` if safe. (infra-guard deep)"
        )),
        None => Decision::Allow,
    }
}
