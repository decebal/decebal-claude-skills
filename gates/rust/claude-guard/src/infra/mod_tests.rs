...
impl Lab {
    fn new() -> Lab {
        ...
        let root =
            std::env::temp_dir().join(format!("claude-guard-lab-{}-{id}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        for d in ["scripts", "sub", "apps", ".claude"] {
            std::fs::create_dir_all(root.join(d)).expect("lab dirs");
        }
        let lab = Lab { root };

        lab.write(
            ".claude/prod-guard-tokens.txt",
    ...
    fn decide_at_depth(&self, cmd: &str, depth: Option<&str>) -> Decision {
        // The guard reads its depth and project root from the environment, which
        // is process-global — so the cases that need a non-default depth are
