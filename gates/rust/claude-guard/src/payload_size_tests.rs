...
fn comment_hygiene_context_stays_small_as_comments_multiply() {
    ...
    // Five comments cost barely more than one: the fixed instruction dominates,
    // which is why it points at CLAUDE.md instead of restating the tests.
...
#[test]
fn an_ask_reason_reaches_the_user_not_the_model() {
    // Free at runtime, so this only guards against it becoming an essay.
