//! `claude-guard trust '<command>'` — vet a wrapper chain, then record YOUR
...
pub fn run(cmd: &str) -> ExitCode {
    if cmd.is_empty() {
        eprintln!("usage: claude-guard trust '<command>'");
    ...
    let Ok(mut file) = opened else {
        eprintln!("claude-guard trust: cannot write {}", path.display());
        return ExitCode::from(1);
    };
