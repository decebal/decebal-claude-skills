...
description: Audit integration-test coverage, find the untested cross-boundary paths, and write tests that match the project's existing conventions — then run and verify they actually exercise the path. Use when the user asks to "audit tests", "check test coverage", "write integration tests", "find test gaps", or "add integration tests". Bakes in the repo's testing philosophy: tests must be un-hangable and must not pass by never running.
---
