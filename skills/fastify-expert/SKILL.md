---
name: fastify-expert
description: Build fast, production-grade Node/Bun HTTP APIs with Fastify — plugin/encapsulation architecture, JSON Schema / TypeBox / Zod validation and serialization, the request lifecycle hooks, decorators, JWT auth, rate limiting, graceful shutdown, and inject() tests. Complements the bun and typescript skills (use bun/bunx). Use when building or reviewing a Fastify server, wiring routes/plugins, adding request validation or response schemas, debugging why a response field vanished, choosing a lifecycle hook, adding auth or an error handler, or writing API integration tests. Triggers on: fastify, fastify-plugin, @fastify/*, withTypeProvider, TypeBox, setErrorHandler, app.inject, preHandler/onRequest hooks, "route not encapsulated", "field missing from response".
---
