...
description: Interactive Turborepo + Bun workspace development — task pipelines in turbo.json, local and remote caching, package filtering, shared tooling packages, cross-service env/ports, and monorepo Docker builds. Use when working in a Turborepo monorepo, editing turbo.json, debugging cache misses or false cache hits, running only affected packages in CI, wiring internal workspace packages, or writing a Dockerfile that builds from a monorepo root. Triggers on: turborepo, turbo.json, monorepo, bun workspaces, --filter, remote cache, dependsOn, turbo prune, affected packages, cache miss.
---
