---
name: bun-testing
description: Write and run tests with Bun's built-in test runner (bun:test). Covers test organization and lifecycle, the expect API, sync/async and snapshot assertions, coverage, and Turborepo caching. Use when writing or running bun tests, configuring bunfig.toml [test], setting coverage thresholds, debugging flaky/order-dependent bun tests, or migrating a Jest/Vitest suite to Bun. Triggers on: "bun test", "bun:test", "mock.module", "toMatchInlineSnapshot", "coverage threshold", "expect().resolves", ".test.ts". For mocking depth see references/mocking.md; for HTTP/DB/env-isolation tests see references/integration.md.
---
