...
description: Operationalizes clean-architecture layering for Rust workspaces — the dependency direction presentation → application → infrastructure ← domain, enforced as a test rather than a convention. Use when creating a new Rust file/module/crate and deciding its layer, reorganizing modules, splitting a god module, setting crate/module visibility, or doing an architecture review. Triggers on: "which layer does this go in", "clean architecture", "layer boundaries", "hexagonal/ports and adapters", "split this module", "god module", "pub or pub(crate)", "architecture test".
---
