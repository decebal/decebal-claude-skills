---
name: rust-quality
description: Set up workspace-level Rust lint gates — clippy pedantic with a battle-tested allow-list, forbidden unsafe with a per-item escape hatch, denied broken rustdoc links, and sorted manifests. Use when starting or hardening a Cargo workspace, adding lint config to Cargo.toml, wiring clippy/rustdoc into CI, or taming lint noise. Triggers on: "set up clippy", "workspace lints", "clippy pedantic", "deny unsafe", "cargo lints table", "cargo sort", "which pedantic lints to allow".
---
