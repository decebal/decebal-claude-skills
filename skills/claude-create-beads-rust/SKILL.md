---
name: claude-create-beads-rust
description: "Convert PRDs to beads for claude execution using beads-rust (cn CLI). Creates an epic with child beads for each user story. Use when you have a PRD and want to use claude with beads-rust as the task source. Triggers on: create beads, convert prd to beads, br beads, cn beads."
---
...
> **Note:** This skill uses the `cn` command from chronis (formerly beads-rust/`br`). If you have the original beads (`bd`) installed instead, use the `claude-create-beads` skill.
...
```bash
cn dep add feature-002 feature-001  # US-002 depends on US-001
cn dep add feature-003 feature-002  # US-003 depends on US-002
```
...
cn dep add feature-002 feature-001
...
cn dep add feature-003 feature-002
```
