//! Deterministic metadata and experiment checks for the `aso-lint` skill.
...
USAGE:
  aso-lint lint --input PATH
  aso-lint experiment --control-conversions N --control-visitors N \
    --variant-conversions N --variant-visitors N [--alpha NUMBER]
  aso-lint template --platform apple|google

Use --input - to read listing JSON from stdin. `validate` remains an alias for `lint`.
";
    ...
    I: IntoIterator<Item = String>,
{
    ...
    match command {
        ...
        "lint" | "validate" => lint_command(&args[1..]),
        "experiment" => experiment_command(&args[1..]),
        "template" => template_command(&args[1..]),
        _ => Err(format!("unknown command {command:?}\n\n{HELP}")),
    }
}

fn lint_command(args: &[String]) -> Result<CliOutcome, String> {
    ...
    let report = lint_listing(&listing)?;
    let valid = report
        .get("valid")
...
fn template_command(args: &[String]) -> Result<CliOutcome, String> {
    let options = parse_options(args)?;
    reject_unknown(&options, &["--platform"])?;
    let template = match required(&options, "--platform")? {
        "apple" => json!({
            "platform": "apple",
            "locale": "en-US",
            "name": "",
    ...
    Ok(success(&pretty(&template)?))
}

pub fn lint_listing(listing: &Value) -> Result<Value, String> {
    let object = listing
        .as_object()
    ...
    match platform {
        "apple" => lint_apple(object, &mut checks, &mut warnings)?,
        "google" => lint_google(object, &mut checks)?,
        _ => return Err("platform must be apple or google".to_owned()),
    }
    ...
    for key in object.keys() {
        if !allowed.contains(key.as_str()) {
            warnings.push(format!("unknown field {key:?} was not linted"));
        }
    }
    ...
    Ok(json!({
        ...
        "checks": checks,
        "warnings": warnings,
        "note": "Structural validation only; store review, ranking, traffic, and conversion performance are not inferred."
    }))
}
...
fn lint_apple(
    ...
    lint_apple_keywords(object, checks, warnings)
}

fn lint_google(object: &Map<String, Value>, checks: &mut Vec<Value>) -> Result<(), String> {
    check_text(
        object,
        checks,
        "ASO-GOOGLE-NAME",
...
fn check_text(
    ...
    let Some(value) = value else {
        if required {
            checks.push(json!({
                ...
                "recommendation": "provide a non-empty value and rerun aso-lint"
            }));
        }
        return Ok(());
    };
    ...
    checks.push(json!({
        ...
        "field": field,
        "status": status,
        "severity": if status == "error" { "high" } else { "info" },
        "used": used,
        "limit": limit,
        "unit": unit.as_str(),
        "message": message,
        "recommendation": if status == "error" {
            "shorten or supply this field, preserve meaning, then rerun aso-lint"
        } else {
...
fn lint_apple_keywords(
    ...
    checks.push(json!({
        "id": "ASO-APPLE-KEYWORDS-MINIMUM",
        "field": "keywords.entries",
        "status": if short.is_empty() { "pass" } else { "error" },
        "severity": if short.is_empty() { "info" } else { "medium" },
        "message": if short.is_empty() {
            "all keyword entries exceed two characters".to_owned()
        } else {
            format!("keyword entries must exceed two characters: {}", short.join(", "))
        },
        "recommendation": "remove low-information entries and spend byte budget on relevant terms"
    }));
...
pub fn analyze_experiment(
    ...
    if control_visitors > MAX_EXACT_F64_INTEGER || variant_visitors > MAX_EXACT_F64_INTEGER {
        return Err("visitor counts must not exceed 2^53 for exact conversion to f64".to_owned());
    }
...
#[cfg(test)]
mod tests {
    use super::{analyze_experiment, lint_listing, run_cli};
    ...
    fn apple_character_boundaries_pass() {
        ...
        let report = lint_listing(&listing).unwrap();
    ...
    #[test]
    fn template_emits_lintable_schema() {
        let outcome = run_cli([
            "template".to_owned(),
            "--platform".to_owned(),
            "google".to_owned(),
        ...
        let template: Value = serde_json::from_str(&outcome.output).unwrap();
        assert_eq!(template["platform"], "google");
        assert!(template.get("short_description").is_some());
    }

    #[test]
    fn cli_help_names_lint_command() {
        let outcome = run_cli(["--help".to_owned()]).unwrap();
        assert_eq!(outcome.exit_code, 0);
        assert!(outcome.output.contains("aso-lint lint"));
    }
}
