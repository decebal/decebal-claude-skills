//! Deterministic, bounded SEO evidence collection for the `claude-seo` skill.
...
const USER_AGENT: &str =
    "decebal-claude-seo/0.1 (+https://github.com/decebal/decebal-claude-skills)";

const HELP: &str = r"claude-seo

USAGE:
  claude-seo audit --input PATH_OR_URL [--base-url URL] [--format json|markdown]
  claude-seo sitemap --input PATH_OR_URL
  claude-seo drift --before AUDIT.json --after AUDIT.json
  claude-seo doctor
...
fn compare_audits(before: &Value, after: &Value) -> Result<Value, String> {
    for (label, value) in [("before", before), ("after", after)] {
        if value["schema_version"].as_u64() != Some(1) || value["page"].as_object().is_none() {
            return Err(format!(
                "{label} is not a claude-seo audit schema version 1"
            ));
        }
    }
    let paths = [
