...
<prime_integration>
**Whenever a PLAN.md, RESEARCH.md, or BRIEF.md is written** (any of the workflows: create-brief, create-roadmap, plan-phase, plan-chunk, research-phase), also register it in the per-repo `.prime/` graph so future planning calls can recall related prior plans and `/run-plan` (or its successor) can attach outcomes.
...
3. For each `@path` reference in the artifact body, ensure a `file` node exists (search by `path` property, create if missing) and `mcp__prime__prime_add_edge { source: <plan>, target: <file>, relation: "references" }`.
