...
brainstorming, skill authoring) — those belong in `~/.claude/skills/` and this repo.
...
```
.claude/skills/<name>/SKILL.md      # project-level — committed, shared with the team
~/.claude/skills/<name>/SKILL.md    # global — your personal, cross-project skills
```

Committing the project skill means every contributor (and every agent) picks up
the same release ritual and the same debugging map. It's documentation that Claude
executes.
